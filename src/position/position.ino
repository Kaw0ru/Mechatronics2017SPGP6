...
int ContainerRST = 1; // RST to find the container
...
void ScanContainer()
{
  if (!Stop)
  {
    ...
    ContainerCmd(P_scanRe, P_scanReHis, &DesiredLoc[0], k, &angle, P_CtlCmd);// give the control cmd
  ...
  if (Close == 1)
  {
    ...
    digitalWrite(laser, LOW); // close the laser head
    myServoGate.write(20); //openGate;
  }

  value = analogRead(laserSensor);   // reads the laser detection value
                     // outputs the laser detection value to the serial monitor

  if ((Close == 1) && (value<300))
  {
    globalState = 4;
  }
